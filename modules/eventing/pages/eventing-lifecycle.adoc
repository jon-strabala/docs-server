...
*Prerequisites*: In order to show the basic life cycle of an Eventing Function, we need a Listen To Location (a source keyspace) and an Eventing storage keyspace.
You must have an Eventing storage keyspace, typically called *metadata*, to be used solely by Eventing -- if it is missing, create it with a minimum size of 100MB.
For the source keyspace we will use the *beer-sample._default._default* sample document set. 

If you don’t already have the sample bucket *beer-sample* (it shuld have a scope of _default and a collection of _default) listed in the *Couchbase Web Console* > *Buckets* page you can load this document set as follows:
...
| Settings
a|
The available settings (by default hidden within a collapsible panel) are:

* *System Log Level:* The granularity at which messages are logged.
...
* *System Log Level*: Determines the granularity at which messages are logged to the common system log messages across all Eventing Functions. 
...
* *Application log location* The directory of the log file for the application or the Function specific log messages named <<function_name>>.log. 
...
In the UI when "Log" is selected these files are combined across all Eventing nodes and displayed.
...
The default value is 1 (the minimum is 1 and the recomended maximum is 64).
...
+
Once the Function completes deployment (about 15 seconds) it will quickly process all of the 7,303 items in the collection because the *Feed boundary* was set to  *Everything* and then await any new mutations. You should see the success count at 7,303 in the Function's basic statistics.
+
image::addfunc_05_deployed_done.png[,100%]
+
Since the example only has a log(....) statement in the *OnUpdate* handler it will merely list items in the collection 'beer-sample', i.e. 7,303 documents. Basically the only thing the function does is print the key or ID of each document.
