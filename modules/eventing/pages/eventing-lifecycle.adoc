...
. The Eventing function is deployed and starts running within a few seconds (about 15). From this point, the defined Function is executed on all existing documents and on subsequent mutations.
+
image::input-output-overview-6.5.png[,%100]
+
Once the Function completes deployment it will quickly process 7,303 items because the *Feed boundary* was set to  *Everything* and then await any new mutations. 
+
image::addfunc_04_newundeployed.png[,100%]
+
...
+
image::addfunc_06_logs_emitted.png[,700]
** Click *Close*.
