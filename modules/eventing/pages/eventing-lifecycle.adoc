...
At this point we could use the Eventing Storage keyspace of _RR100._default._default_, for all Eventing Functions but we might want to split this up by tenants as follows:
...
| Bindings
a|
A binding is a construct that allows separating environment specific variables (example: bucket names, external endpoint URLs, constants) from the handler source code.  Currently Eventing Functions support the following types bindings:
