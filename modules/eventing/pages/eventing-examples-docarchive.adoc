...
For this example, three (3) buckets *'bulk'*, '*archive*', and *'rr100'* are required where the later is intended to be 100% resident.  
...
Within the buckets we need three (3) keyspaces *'bulk.data.source'*, *'archive.data.target'*, and *'rr100.eventing.metadata'* 
...
The 'bulk' bucket will have a 'Bucket Max Time-To-Live' or TTL of 600 seconds.
...
. Update the advanced settings for the "bulk" bucket from the *Couchbase Web Console* > *Buckets* page
** Expand the bucket "bulk" by clicking on it's row. 
 ...
 ** [Optional Step] Enter text *Function that archives all documents in a collection from a collection in a bucket awith a TTL set*, in the *Description* text-box.
 ** For the *Settings* option, use the default values.
 ** For the *Bindings* option, add two bindings.
 ...
 *** For the second binding, select "bucket alias", specify *tgt* as the "alias name" of the collection, 
 select *archive*, *data*, and *target* as the associated keyspace, and select "read and write" for the access mode.
...
collection will have 1280 documents related to the Eventing function and "about" 2 x 1,968 + 1 additional documents related to the active timers.
