...
. Potential issue with the URL string
+
...
+
image::debug_websocket_disconnected.png[,100%]
+
...
=== The Debugging Window (or Tab) in the Chrome web browser.

. Once the pasted URL is processed by Chrome the resulting debugger window will look something like the following:
+
