...
Timers are asynchronous compute, which offers Eventing Functions the ability to execute in reference to wall-clock events.
...
* The Metadata bucket (or Eventing Storage bucket) stores information about timers and its association with an Eventing Function's handler.
...
* The default timer context size, "timer_context_size", can be increased globally via the REST API through version 6.6.1. 
* Starting with 6.6.2 timer context size may be adjusted up or down on a per Function handler basis in the Function's settings. [.status]#Couchbase Server 6.6.2#
